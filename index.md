...
### Main Ideas
To summarize, locally weighted linear regression builds off of linear regression to provide a means for creating nonlinear models. This is done by adding weights to the linear regression equation that place a greater importance on nearby values for a prediction. These weights are determined through kernels, which are mathematical functions such a Gaussian distribution. With these kernels, locally weighted linear regression builds a model by iterating over all datapoints and a building a relatively small linear regression model for the current neighborhood of datapoints. These local linear regressions are then combined to form the overall model. The width of the kernel is adjusted through the tau hyperparameter, and this affects the fit of the model by making the neighborhoods larger or smaller. A larger value of tau will make the model closer to a normal linear regression model, whereas a smaller tau will fit the model closer to the data, but with the risk of overfitting. Weighted linear regression is a good modeling technique to employ for numerical data that does not follow a linear trend.
